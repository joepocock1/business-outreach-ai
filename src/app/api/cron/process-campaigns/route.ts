...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Cron] Current time: ${now.toISOString()}, Hour: ${currentHour}, Day: ${currentDay}, Weekday: ${isWeekday}`);
    ...
    for (const campaign of campaigns) {
      ...
      try {
        ...
        if (currentHour < campaign.sendWindowStart || currentHour >= campaign.sendWindowEnd) {
          console.log(`[Cron] Campaign ${campaign.name}: Outside send window (${campaign.sendWindowStart}-${campaign.sendWindowEnd}), current: ${currentHour}`);
          continue;
        }

        // Check weekday constraint
        if (campaign.sendWeekdaysOnly && !isWeekday) {
          console.log(`[Cron] Campaign ${campaign.name}: Weekdays only, today is weekend`);
          continue;
        }
        ...
        console.log(`[Cron] Campaign ${campaign.name}: Rate limits - hourly: ${availableHourly}, daily: ${availableDaily}, available: ${availableToSend}`);

        if (availableToSend === 0) {
          console.log(`[Cron] Campaign ${campaign.name}: Rate limit reached`);
          continue;
        }
        ...
        console.log(`[Cron] Campaign ${campaign.name}: Found ${queuedEmails.length} queued emails to process`);
        ...
        if (queuedEmails.length === 0) {
          ...
          if (remainingQueued === 0) {
            console.log(`[Cron] Campaign ${campaign.name}: No more queued emails, marking as completed`);
        ...
        for (const email of queuedEmails) {
          ...
          try {
            ...
            if (unsubscribed) {
              console.log(`[Cron] Email ${email.id}: Lead ${email.lead.email} is unsubscribed`);
            ...
            // Send via Resend
            console.log(`[Cron] Sending email ${email.id} to ${email.lead.email}...`);
            ...
            console.log(`[Cron] Email ${email.id}: Sent successfully, Resend ID: ${sendResult.data?.id}`);
            result.sent++;

          } catch (error) {
            console.error(`[Cron] Email ${email.id}: Send failed`, error);
        ...
        if (remainingAfterProcessing === 0) {
          console.log(`[Cron] Campaign ${campaign.name}: Completed after processing`);
          await db.campaign.update({
            where: { id: campaign.id },
            data: {
              status: "Completed",
        ...
        console.error(`[Cron] Campaign ${campaign.id} error:`, campaignError);
      }
    ...
    console.log(`[Cron] Processing complete. Totals:`, totals);
